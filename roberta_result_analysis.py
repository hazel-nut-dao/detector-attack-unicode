...
results = []
result_file_list = [filename for filename in os.listdir(folder) if filename.startswith('roberta_original')]
...
results = []
result_file_list = [filename for filename in os.listdir(folder) if filename.startswith('roberta_homoglyph')]
for file in result_file_list:
    in_file = open('result/'+file, "r")
    sublist = json.load(in_file)
    in_file.close()
